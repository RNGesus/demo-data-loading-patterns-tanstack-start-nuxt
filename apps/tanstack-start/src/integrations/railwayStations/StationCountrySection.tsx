...
function AppTypeIcon({ type }: { type: types.AppType }) {
  ...
  // oxlint-disable-next-line no-underscore-dangle no-unused-vars
  const _exhaustiveCheck = type satisfies 'ios'
  return '🍏'
}
    ...
    <section className="card bg-base-300 flex flex-col gap-2 shadow-sm">
      <div className="card-body">
        <h2 className="card-title">
          {country.name} <small className="leading-relaxed">({country.code})</small>
        </h2>
        {children}
      </div>
    </section>
